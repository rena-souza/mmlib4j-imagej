 ...
 */
public class Reconstruction_by_erosion extends AbstractReconstructionPlugin {
	
	double raio;
	
	@Override
	public String getPluginName() {
		return "Reconstruction by erosion";
	}
	
	@Override
	public GrayScaleImage reconstruct(GrayScaleImage imgF, GrayScaleImage imgG) {
		ReconstructionMorphological rec = new ReconstructionMorphological(imgF, AdjacencyRelation.getCircular(raio), false);
		GrayScaleImage imgOut = rec.reconstructionByErosion(imgG);
		return imgOut;
	}
	...
	public boolean initParameters() {
		...
		if (wList == null || wList.length < 2) {
			IJ.showMessage("Morphological reconstruction based on marked image",
					"There must be at least two windows open");
			return false;
		}
